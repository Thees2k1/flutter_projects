...
import 'package:drawing_app/src/src.dart';
...
class DrawingCanvas extends StatefulWidget {
  ...
  final CurrentStrokeNotifier currentStrokeListenable;
  final DrawingCanvasOptions options;
  final ValueChanged<Stroke?>? onDrawingStrokeChanged;
...
class _DrawingCanvasState extends State<DrawingCanvas> {
  ...
  CurrentStrokeNotifier get _currentStroke => widget.currentStrokeListenable;
...
class _DrawingCanvasPainter extends CustomPainter {
  final ValueNotifier<List<Stroke>>? strokesListenable;
  final CurrentStrokeNotifier? strokeListenable;
  ...
  void paint(ui.Canvas canvas, ui.Size size) {
    //Draw the background image if having one
    ...
    for (final stroke in strokes) {
      ...
      final paint = Paint()
        ..color = stroke.color.withAlpha((255 * stroke.opacity).ceil())
      ...
      switch (stroke) {
        case NormalStroke _:
          if (stroke.points.length == 1) {
            ...
            canvas.drawCircle(center, radius, paint..style = .fill);
            break;
          }
